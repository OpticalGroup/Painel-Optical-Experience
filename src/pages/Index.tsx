...
import { Users, GraduationCap, TrendingUp, DollarSign, ArrowRight, Calendar, MapPin, Upload, FileSignature, Clock, Target, CalendarIcon } from "lucide-react";
...
const Index = () => {
  ...
  const { data: analytics, isLoading: isLoadingAnalytics } = useEnrollmentAnalytics(dateRange);
  ...
  // Calculate totals com base nos cohorts filtrados
  const totalEnrolled = filteredCohorts.reduce((sum, c) => sum + (c.stats?.enrolled_count || 0), 0);
  const totalCapacity = filteredCohorts.reduce((sum, c) => sum + c.capacity, 0);
  const totalPaid = filteredCohorts.reduce((sum, c) => sum + (c.stats?.paid_count || 0), 0);
  const totalRevenue = filteredCohorts.reduce((sum, c) => sum + (c.stats?.total_revenue || 0), 0);
  ...
  const paidWithoutSignature = useMemo(() => {
    return filteredCohorts.reduce((sum, c) => {
      ...
      return sum + (paidCount - signedCount);
    }, 0);
  }, [filteredCohorts]);
          ...
          {!isLoadingAnalytics && analytics && (
            <section className="px-8 pb-6">
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <ExportButton type="sales-reps" label="Vendedores" size="sm" />
                  <ExportButton type="sources" label="Origens" size="sm" />
                  <ExportButton type="conversion" label="Conversão" size="sm" />
                </div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <RankingCard
                  ...
                  items={analytics.salesReps.map(rep => ({
                    ...
                    subtitle: `R$ ${(rep.totalRevenue / 1000).toFixed(1)}k`,
                  }))}
                />
                <RankingCard
                ...
                />
                <ConversionAnalysis data={analytics.conversions} />
              </div>
            </section>
          )}
